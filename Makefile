#############################################################################
# Library Makefile for building: libstats
# Ron Brash, 2016
#############################################################################
LIBBASENAME = wiringPi
DEVLIBBASENAME = wiringPiDev
MAJVER = 1
MINVER = 0
PATCHVER = 0
...
TARGET0D      = lib$(DEVLIBBASENAME).so
TARGET1D      = lib$(DEVLIBBASENAME).so.$(MAJVER)
TARGET2D      = lib$(DEVLIBBASENAME).so.$(MAJVER).$(MINVER)
TARGETXD      = lib$(DEVLIBBASENAME).so.$(MAJVER).$(MINVER).$(PATCHVER)
...
$(TARGET):  $(UICDECLS) $(OBJECTS) $(SUBLIBS) $(OBJCOMP)  
	...
	-ln -s $(TARGET) $(TARGET1)
	-ln -s $(TARGET) $(TARGET2)
	-ln -s $(TARGET) $(TARGETXD)
	-ln -s $(TARGET) $(TARGET0D)
	-ln -s $(TARGET) $(TARGET1D)
	-ln -s $(TARGET) $(TARGET2D)
...
distclean: clean
	-$(DEL_FILE) $(TARGET) 
	-$(DEL_FILE) $(TARGET0) $(TARGET1) $(TARGET2) $(TARGETA) $(TARGET0D) $(TARGET1D) $(TARGET2D) $(TARGETXD)
...
install:  
	...
	$(COPY_FILE) -a lib$(DEVLIBBASENAME).so* $(DESTDIR)/lib
